...
void poster_host(unsigned char* in, int w, int h, 
...
void run_test(unsigned char* in, int w, int h, 
    ...
    poster_host(in, w, h, &out_ok, &ow_ok, &oh_ok, &t);

    // compute enlarge on device
    poster(in, w, h, out, ow, oh, dev, &td);
