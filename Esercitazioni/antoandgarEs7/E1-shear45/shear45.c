             ...
             double *t, clut_device *dev)
{
        ...
        *out = malloc((w+h)*h*sizeof(unsigned char));
        ...
        if (!din)
                clut_panic("failed to allocate input matrix on device memory");

        // allocate output matrix on device
        dout = clCreateBuffer(dev->context,
                              CL_MEM_WRITE_ONLY,
                              h * (w+h) * sizeof(unsigned char), NULL, NULL);
        if (!dout)
                clut_panic("failed to allocate output matrix on device memory");
        ...
        err = clEnqueueReadBuffer(dev->queue, dout, CL_TRUE, 0,
                                  h * (w+h) * sizeof(unsigned char), *out, 0, NULL, NULL);
